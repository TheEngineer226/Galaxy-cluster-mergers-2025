...
# --- Default Parameters ---
DEFAULT_PARAMETERS = {
    "data_location": "sims_data/R1.5_v2400_b250/Data_000074",
    "sphere_radius": 3,
...
def main(data: str = DEFAULT_PARAMETERS['data_location'], sphere_radius: float = DEFAULT_PARAMETERS['sphere_radius'], sphere_radius_units: str = DEFAULT_PARAMETERS['sphere_radius_units']):
    ...
    shock_temp_threshold = ds.quan(15, "keV").to("K", equivalence="thermal")
    ...
    if shock_mask_field not in ds.field_list:
        ...
        ds.add_field(name=shock_mask_field, sampling_type="cell", function=_shock_mask, units="dimensionless")
    ...
    if shock_velocity_field not in ds.field_list:
        def _shock_velocity(field, data):
            ...
            return mask * vel_mag
        ds.add_field(name=shock_velocity_field, sampling_type="cell", function=_shock_velocity, units="km/s")
    ...
    tf.map_to_colormap(log_vel_bounds[0], log_vel_bounds[1], colormap="magma", scale_func=alpha_func)
    ...
    # --- NEW: Set custom camera orientation ---
    # Define the custom line of sight and north vectors
    L = np.array([0.94, -0.10, 0.31]) / np.linalg.norm(np.array([0.94, -0.10, 0.31]))
    N = np.array([L[1], -L[0], 0]) # A vector perpendicular to L

    cam = sc.camera
    # Position the camera far away from the center of mass along the L vector
    cam.position = c + ds.arr(L, 'dimensionless') * ds.quan(15, 'Mpc')
    cam.focus = c # Look at the center of mass
    cam.north_vector = N
    cam.zoom(2.5) # Adjust zoom as needed
    ...
    save_and_show_img(output_dir + "shock_render.png", 4.0, (1, 2, 2), "Shock Velocity (Custom View)", is_transfer_function=False, scene=sc)
